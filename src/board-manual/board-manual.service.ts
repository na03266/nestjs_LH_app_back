...
@Injectable()
export class BoardManualService {
    constructor(
        @InjectRepository(BoardManual) private readonly noticeRepository: Repository<BoardManual>,
    ...
    async createPost(
        ...
        const now = new Date();
        const entity = manager.create(BoardManual, {
            wrNum: wrNum ?? '',
            wrReply: '',
            wrParent: 0,
            wrCommentReply: '',
        ...
        const saved = await manager.save(BoardManual, entity);
        await manager.update(BoardManual, {wrId: saved.wrId}, {wrParent: saved.wrId});
    ...
    async replyPost(
        ...
        const now = new Date();
        const entity = qr.manager.create(BoardManual, {
            wrNum: parent.wrNum,
            wrReply: reply,
        ...
        const saved = await qr.manager.save(BoardManual, entity);

        await qr.manager.update(BoardManual, {wrId: saved.wrId}, {wrParent: saved.wrId});
        return saved.wrId;
    }
    ...
    private async getNextCommentLetter(
        ...
        const qb = qr.manager
            .createQueryBuilder(BoardManual, 'w')
            .select(
    ...
    async createComment(
        ...
        if (commentId) {
            // 대댓글: 대상 댓글 조회
            const base = await qr.manager.findOne(BoardManual, {
        ...
        await qr.manager.update(BoardManual, {wrId: parent.wrId}, {
    ...
    async findOne(wrId: number) {
        ...
        const lite = files.map((e) => ({
            url: `http://${ip}/data/file/comm10_1/${e.bfFile}`,
            fileName: e.bfSource,
        }))
